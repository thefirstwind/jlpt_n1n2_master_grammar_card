...
| 文件 | 说明 |
|------|------|
| [N1N2_语法复习.html](N1N2_语法复习.html) | **打开此文件** 即可复习（Chrome 等） |
...
双击或在浏览器中打开 **`N1N2_语法复习.html`**。工具栏 **第1/2/3遍** 决定本次打分记入哪一遍；每遍各自保存熟悉/不熟（非全局一条状态）。左侧 **遍** 列三格对应三遍。上次学到的词条与遍次会自动恢复（本机 `localStorage`）。可选 **邮箱云端同步**（Supabase，Mac/iPad 共用）见 [SYNC_README.md](SYNC_README.md)。
